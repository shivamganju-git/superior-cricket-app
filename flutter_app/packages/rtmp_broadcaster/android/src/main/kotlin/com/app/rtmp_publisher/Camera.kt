...
//    private val glView: FlutterGLSurfaceView
    ...
    private var imageFilter: AndroidViewFilterRender? = null
...
//                , imageStreamReader!!.surface
        ...
        imageStreamChannel.setStreamHandler(
                object : EventChannel.StreamHandler {
                    override fun onListen(o: Any, imageStreamSink: EventSink) {
                        setStreamObjectStreamImageAvailableListener(imageStreamSink)
                    }
    ...
    private fun setStreamObjectStreamImageAvailableListener(imageStreamSink: EventSink) {
...
//    }
    ...
    fun setOverlay(imageBytes: ByteArray, result: MethodChannel.Result) {
        if (!useOpenGL) {
            result.error("OpenGLNotEnabled", "Overlay only supported with enableAndroidOpenGL: true", null)
            return
        }
        ...
        try {
            ...
            if (bitmap == null) {
                result.error("DecodeError", "Failed to decode image bytes", null)
                return
            }
            ...
            activity.runOnUiThread {
                try {
                    ...
                    if (imageFilter == null) {
                        imageFilter = AndroidViewFilterRender()
                        rtmpCamera.getGlInterface().setFilter(imageFilter)
                    }
                    imageFilter?.setView(frameLayout)
                    
                    result.success(null)
                } catch (e: Exception) {
                    result.error("OverlayUIError", e.message, null)
                }
            }
        } catch (e: Exception) {
            result.error("OverlayError", e.message, null)
        }
    }
}
